...
#
# Release tags here point at a digest-pin commit that lives off main, so one
# release tag is never an ancestor of the next and `git describe` cannot find
# the previous one — release.yml always passes previous-tag explicitly. The
# two-dot range still works: it walks from the previous pin commit's merge base
# with the new tag, so exactly the commits merged between the two releases are
# listed. Pin and version-bump commits are filtered as release noise.
#
...
# --match restricts to semver tags — without it, git describe would pick up
# the floating major tag (v0) and truncate the range.
PREV_TAG="${2:-$(git describe --tags --abbrev=0 --match "v[0-9]*.[0-9]*.[0-9]*" "$NEW_TAG"^ 2>/dev/null || echo "")}"
...
# Release noise: the version-bump commit ("release: vX.Y.Z") and the
# digest-pin commit ("chore(release): pin image to sha256:…").
release_pat = re.compile(r'^(release|chore\(release\)):')
