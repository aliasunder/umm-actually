...
6. Drops non-findings (see [Non-finding filter](#non-finding-filter)) and findings on files the model was never given (see [Unknown-file filter](#unknown-file-filter)), then on re-runs deduplicates against previously posted bot comments (two-tier: positional match by hidden HTML anchor, or content match by title similarity within 50 lines)
