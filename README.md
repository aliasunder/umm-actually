...
| `request_timeout_seconds` | `600`                         | Per-attempt timeout for a single model request, in seconds. A timed-out attempt aborts the HTTP call and advances the retry/fallback ladder instead of hanging the job                                                                                                                                                                                                                                                                                                                                           |
...
| `conventions_file`        | `AGENTS.md`                   | Repo-relative path to the conventions file included in the prompt (truncated at ~8000 tokens). When it also changed in the PR and this section carries the whole file, the changed-files copy is included diff-only so its text is not sent twice; when this section truncates, the changed-files copy stays full so the tail is not lost. When `priority_docs` reads the full file (because the truncated section did not carry it whole), the truncated conventions section is suppressed to avoid duplication |
...
| `priority_docs`           | `README.md`                   | Comma-separated repo-relative paths always included in review context, independent of `trace_related_files`. A doc whose full text is already in context — as a full-content changed file, a related file, or the conventions file (when that section carries the whole file) — is not read again. A changed file included as diff-only is still read here so its full text reaches the model. Shares the doc token budget with mention-matched docs when both are active. Empty = disabled                      |
