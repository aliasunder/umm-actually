...
| `priority_docs`           | `README.md`                   | Comma-separated repo-relative paths always included in review context, independent of `trace_related_files`. A doc already in context as a changed file, a related file, or the conventions file (when that section carries the whole file) is not read again — its full text is never sent twice. Shares the doc token budget with mention-matched docs when both are active. Empty = disabled |
