...
| `conventions_file`        | `AGENTS.md`                   | Repo-relative path to the conventions file included in the prompt (truncated at ~8000 tokens). When it also changed in the PR and this section carries the whole file, the changed-files copy is included diff-only so its text is not sent twice; when this section truncates, the changed-files copy stays full so the tail is not lost                                                                                                                                                   |
